...
for i in `seq $si $ei | shuf`; do
    ...
    # ${s:$i:1}: variable expansion, take substring start from $i with length 1, this operator in bash uses 0-based coordinate
...
**说明**：在 bash 脚本中，双括号 `(())` 和 `$(())` 一样用来定义算术操作，区别在于它不会将结果返回，而是直接对变量值进行修改。例如 `((val++))` 会将变量 `$val` 的数值加 1。其他语言里常见的写法，如 `((val+=1))` 或 `((val=val+1))` 也都是可以的。
