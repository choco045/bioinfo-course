...
BED 文件极其灵活，它**最少只需要前 3 列**就能正常工作，但**最多可以有 12 列**。

### 6列 BED 文件格式总结 (基于 GFF 转换)

| BED 列号 | BED 列名 | 对应 GFF 字段 | 含义说明 |
| :--- | :--- | :--- | :--- |
| **第 1 列** | **chrom** | `$1` | 染色体名称（如 `chr22`, `chrX`），两者定义完全一致。 |
| **第 2 列** | **chromStart** | `$4 - 1` | 区间起始位置。GFF 是从 1 开始的，BED 是从 0 开始的，所以必须减 1。 |
| **第 3 列** | **chromEnd** | `$5` | 区间终止位置。转换为前闭后开区间，数值刚好不需要变化。 |
| **第 4 列** | **name** | `$9` (Attribute) | **该区间的名称**。在转换时，我们通常把 GFF 附加信息中的名字放到这里。 |
| **第 5 列** | **score** | `$6` (Score) | **评分 (0-1000)**。如果没有评分，通常用 `.` 代替。 |
| **第 6 列** | **strand** | `$7` (Strand) | **正负链方向**。`+` 表示正链，`-` 表示负链。 |

---

### ⚠️ 实际操作中的一个“避坑”提示：关于 `$9`

虽然理论上 BED 的第 4 列对应 GFF 的 `$9`，但在实际敲代码转换时，**极其不建议直接把整个 `$9` 原封不动地打印过去**。

* **为什么？** 因为 GFF 的 `$9`（附加属性列）是一大长串键值对，长这样：`gene_id "ENSG0000"; gene_name "ACTB"; transcript_id "ENST0000";...`
* **后果：** 如果把这么长的一串直接塞进 BED 的 name 列，不仅文件会变得极其臃肿，而且中间有很多空格，可能会导致后续使用 `bedtools` 等软件时报错（因为 BED 默认以制表符或空格分列）。
* **正确的做法：** 我们在转换时，通常会通过 `awk` 或 `sed` 等命令做一下处理，**只把 `$9` 里面的那个“基因名字（比如 ACTB）”单独提取出来**，放在 BED 的第 4 列。

如果你想写一个提取 6 列的 `awk` 命令，并且直接拿整个 `$9` 占位，代码长这样：
```bash
awk -v OFS="\t" '{print $1, $4-1, $5, $9, $6, $7}' input.gff > output.bed
```
*(注意：加上 `-v OFS="\t"` 是为了确保生成的 BED 文件严格使用制表符 `Tab` 分隔每一列，这是保证 BED 文件格式正确的关键！)*
