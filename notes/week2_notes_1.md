...
```
`-k`允许你指定使用哪些字段作为排序键，可以指定起始字段和结束字段，甚至可以指定字段内的字符偏移。  
格式：`-k 起始字段[.起始字符],结束字段[.结束字符]`，并可以附加排序选项（如 n 表示数值排序）。  
不加`-k`即作为字符串整行排序。  
...
sort [选项] [文件...]
```
默认按字典序排序整行，结果输出到标准输出。

### 核心选项
| 选项 | 作用 | 示例 |
|------|------|------|
| `-n` | 按数值排序 | `sort -n file.txt` |
| `-r` | 逆序（降序） | `sort -rn file.txt` |
| `-k` | 指定排序字段（列） | `sort -k 2,2n file.txt`（按第2列数值） |
| `-t` | 指定字段分隔符（默认空白） | `sort -t ',' -k 3,3n data.csv` |
| `-u` | 去重（连续相同行只留一条） | `sort -u file.txt` |
| `-o` | 输出到文件 | `sort -o out.txt in.txt` |
| `-f` | 忽略大小写 | `sort -f file.txt` |
| `-b` | 忽略行首空白 | `sort -b file.txt` |
| `-h` | 按人类可读数值（K/M/G）排序 | `sort -h sizes.txt` |
| `-V` | 按版本号排序 | `sort -V versions.txt` |
| `-c` | 检查是否已排序 | `sort -c file.txt` |
| `-m` | 合并已排序文件 | `sort -m a.txt b.txt` |
| `-R` | 随机排序 | `sort -R file.txt` |
...
### 注意事项
- 字段编号从 **1** 开始。
- 默认分隔符是空白（空格/制表符），连续空白视为一个字段。
- 多个 `-k` 按顺序决定排序优先级。
- 可用 `-s` 保持稳定排序（相同键的行保持原顺序）。
