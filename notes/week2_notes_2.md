...
# split是awk的一个内建的函数，用于根据指定分隔符分割字符串。它接受输入字符串，输出列表和分隔符三个参数
...
- 从 GTF 文件中提取所有 **基因（gene）** 的特征，解析出基因名称，并计算每个基因的长度（结束位置 - 开始位置 + 1），最后显示前 10 条结果。
- `split($10, x, ";")`
  - GTF 文件的第 10 列是**属性列**，包含以分号分隔的多个键值对，例如：  `gene_id "ENSG000001"; gene_name "TP53"; ...`  
  - `split` 函数将第 10 列的内容按分号 `;` 分割，并将分割后的片段存入数组 `x`。  
  - `x[1]` 就是第一个片段，通常是 `gene_id "ENSG000001"` 或类似的标识。
- `name = x[1]`
  - 将数组的第一个元素赋值给变量 `name`，此时 `name` 还包含引号，例如：`gene_id "ENSG000001"`。
- `gsub("\"", "", name)`
  - `gsub` 是 awk 的全局替换函数。  
  - `\"` 表示一个双引号字符（需要使用反斜杠转义）。  
  - 该命令将 `name` 中的所有双引号替换为空字符串（即删除所有双引号）。处理后，`name` 变成类似 `gene_id ENSG000001` 的形式（去掉了引号）。
- `print name, $5-$4+1`
  - `$5` 和 `$4` 分别是 GTF 的第 5 列（结束位置）和第 4 列（开始位置）。  
  - GTF 坐标是 1-based 闭区间，所以长度计算公式为 `$5 - $4 + 1`。  
  - 打印处理后的基因名称和该基因的长度。
- 示例输出（假设）
```
gene_id ENSG000001 2345
gene_id ENSG000002 5678
...
```
...
> less 1.txt   # 浏览文件，按 q 退出
> vi 1.txt     # 打开编辑，按 i 开始修改，按 Esc 然后 :wq 保存退出
...
vi run.sh
```
> `.sh`：扩展名表示这是一个**Shell 脚本**（Shell 是一种命令行解释器，如 Bash）。所以 run.sh 就是一个“可运行的 Shell 脚本文件”。
...
```
第一行的 `#!/bin/bash` 告诉操作系统用 `/bin/bash` 程序作为解释器来执行脚本中的命令  
如果脚本没有执行权限或直接 `bash run.sh` 运行，这一行不是必需的；但如果直接 `./run.sh` 运行，系统会读取这一行来确定使用哪个解释器。  
这个脚本是一个简单的 **Shell 脚本**，用于从当前目录下所有扩展名为 `.gtf` 的文件中，提取所有 `exon` 特征的长度，并输出最长的 3 个长度值。下面逐部分解释：  
- `grep exon *.gtf`
  - `*.gtf` 是一个通配符，匹配当前目录下所有以 `.gtf` 结尾的文件（例如 `1.gtf`、`2.gtf` 等）。
  - 该命令会将所有 `.gtf` 文件中包含 `exon` 的行输出到标准输出（屏幕），并通过管道 `|` 传递给下一个命令。
- `sort -n`
  - `sort` 命令对输入的行进行排序, `-n` 选项表示按**数值大小**排序（而不是默认的字典序）。
  - 所有长度值会从小到大排列。
